  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 shadow-sm no-print">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between gap-4">
        <button type="button" onClick={() => handleSelect('dashboard')} className="flex items-center gap-3 min-w-0 text-left cursor-pointer">
          <HospitalLogo size="md" />
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <span className="font-black text-slate-900">SIDOKTER SOEGIRI</span>
              <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-800 border border-emerald-200">
                <ShieldCheck className="inline w-3 h-3 mr-1" />Admin
              </span>
            </div>
            <p className="text-xs text-slate-500 font-medium">RSUD Dr. Soegiri Lamongan</p>
          </div>
        </button>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer ${
                  active ? 'bg-white text-emerald-800 shadow-sm font-black' : 'text-slate-600 hover:bg-white/70'
                }`}
              >
                <Icon className="w-4 h-4" />
                <span>{item.label}</span>
                {typeof item.count === 'number' && item.count >= 0 && (
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-md font-extrabold ${active ? 'bg-emerald-50 text-emerald-800' : 'bg-slate-200 text-slate-600'}`}>
                    {item.count}
                  </span>
                )}
              </button>
        ...
        <div className="flex items-center gap-2">
          <div className="hidden sm:flex items-center gap-2.5 px-3 py-1.5 rounded-2xl bg-slate-50 border border-slate-200/80">
            <div className="w-7 h-7 rounded-xl bg-slate-900 text-white flex items-center justify-center font-black text-xs">
              {(userSession?.name || 'A')[0].toUpperCase()}
            </div>
            <div className="text-left hidden md:block">
              <div className="text-xs font-black text-slate-800 leading-tight">{userSession?.name || 'Administrator'}</div>
              <div className="text-[10px] text-emerald-700 font-bold">Super Admin</div>
            </div>
          </div>
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <nav className="lg:hidden p-3 border-t border-slate-100 space-y-1.5 bg-white shadow-xl animate-in slide-in-from-top-2 duration-150">
          {adminItems.map((item) => {
            const Icon = item.icon;
            const active = activeTab === item.id;
            return (
