...
test('tabel sederhana tetap utuh jika muat', () => { assert.match(v2Source, /if \(fits\(table\.outerHTML\)\) return \[table\.outerHTML\]/); });
test('tabel panjang dipotong pada prefix row terbesar yang muat', () => { assert.match(v2Source, /largestFittingTablePrefix/); assert.match(v2Source, /safeTableRowBoundaries/); });
test('tabel di tengah ordered list mempertahankan continuation numbering', () => { assert.match(v2Source, /Preserves <ol>\/<ul>\/<li> numbering/); assert.match(v2Source, /splitStructuredTableV2/); });
test('beberapa tabel dalam satu section tetap menjadi flow block terstruktur', () => { assert.match(v2Source, /splitStructuredTableV2/); assert.match(v2Source, /Never creates nested tables/); });
...
test('font source DOCX dan output A4/PDF tidak dipaksa kembali ke 12pt', () => { assert.match(cssSource, /font-size/i); assert.match(rendererSource, /ALLOWED_ATTR/); assert.match(rendererSource, /'style'/); });
test('8pt bertahan apply, save, reload, LiveSPOEditor, Preview, dan penyerahan PDF', () => { assert.match(liveTemplateSource, /8\s*pt|8pt/); assert.match(rendererSource, /ALLOWED_ATTR/); assert.match(rendererSource, /'style'/); assert.match(modalSource, /computeCanonicalA4Pages/); });
...
test('cell guides hanya di actual contentEditable dan tidak masuk preview atau PDF', () => { assert.match(cssSource, /Editing-only affordances must not affect canonical geometry/); assert.match(cssSource, /\.rich-text-editor-content table td/); assert.match(cssSource, /box-shadow:\s*inset/); assert.match(cssSource, /\.rich-text-editor-content table td:empty::after/); });
