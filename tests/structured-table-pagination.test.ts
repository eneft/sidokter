...
test('font source DOCX dan output A4/PDF tidak dipaksa kembali ke 12pt', () => { assert.match(cssSource, /font-size/i); assert.match(rendererSource, /font-size/i); });
test('8pt bertahan apply, save, reload, LiveSPOEditor, Preview, dan penyerahan PDF', () => { assert.match(liveTemplateSource, /8\s*pt|8pt/); assert.match(rendererSource, /font-size/i); assert.match(modalSource, /computeCanonicalA4Pages/); });
