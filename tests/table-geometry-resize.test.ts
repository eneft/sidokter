...
test('outer border hit testing exposes horizontal table edges only', () => {
  ...
  assert.equal(tableOuterEdgeAtPoint(rect as DOMRect, 300, 202, 7), null);
  assert.equal(tableOuterEdgeAtPoint(rect as DOMRect, 300, 598, 7), null);
...
test('actual horizontal outer table borders are draggable and committed as one document edit', () => {
  assert.match(geometry, /type OuterTableEdge = 'left' \| 'right'/);
  ...
  assert.match(geometry, /pointercancel[\s\S]*cancel/);
  assert.match(geometry, /restoreTableSnapshot/);
  assert.match(geometry, /sidokter:table-rebind/);
});
...
test('AutoFit remains reversible, clears manual position and manual resize disables it', () => {
  assert.match(editor, /toggleTableAutoFit/);
  assert.match(editor, /applyTableAlignment\(table, currentAlign\)/);
  assert.match(editor, /table\.style\.tableLayout = 'auto'/);
  assert.match(editor, /ensureLogicalColumns\(table\)/);
