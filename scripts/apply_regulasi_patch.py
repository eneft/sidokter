...
def replace_once(path: str, old: str, new: str, count: int = 1):
    ...
    if found < count:
        raise RuntimeError(f'{path}: expected at least {count} matches, found {found}: {old[:120]!r}')
...
def replace_all_exact(path: str, old: str, new: str, expected: int):
    ...
    found = text.count(old)
    if found != expected:
        raise RuntimeError(f'{path}: expected exactly {expected} matches, found {found}: {old[:120]!r}')
...
old_read_guard = """    const isAdmin = normalizeRole(context.user.role) === 'admin';
...
"""
new_read_guard = """    const isAdmin = normalizeRole(context.user.role) === 'admin';
    const isStructural = Array.isArray(context.user.badges) && context.user.badges.some(b => String(b).trim().toUpperCase() === 'STRUKTURAL');
    const hasGlobalAccess = Boolean(context.user.sopGlobalAccess || context.user.divisionCode === 'ALL');
    const keys = storageAccessKeys(context.user);
...
"""
replace_all_exact('functions/index.js', old_read_guard, new_read_guard, 2)
...
old_mou_block = """        {/* TAB 4: MOU (Halaman Terpisah) */}
        {activeTab === 'mou' && (
          <MOUPage
...
"""
replace_once('src/components/UserView.tsx', old_mou_block, new_mou_block)

# ---------- New regulation service ----------
(ROOT / 'src/lib/regulationService.ts').write_text(r'''/**
 * REGULASI SERVICE
...
# ---------- New regulation page ----------
(ROOT / 'src/components/RegulasiPage.tsx').write_text(r'''import React, { useEffect, useMemo, useRef, useState } from 'react';
import {
  ArrowLeft,
  BookOpen,
  Calendar,
...
# Remove this one-shot patch machinery from the resulting feature commit.
workflow = ROOT / '.github/workflows/apply-regulasi-once.yml'
if workflow.exists():
    workflow.unlink()
