...
export interface RichTextEditorHandle {
  ...
  applyFontSize: (fontSize: '8pt' | '10pt' | '12pt') => void;
...
export const RichTextEditor = React.forwardRef<RichTextEditorHandle, RichTextEditorProps>(function RichTextEditor({
  ...
  const updateActiveFormatting = useCallback(() => {
    ...
    try {
      ...
      let fontSize: '8pt' | '10pt' | '12pt' | null = null;
      if (selection?.rangeCount && editorRef.current.contains(selection.anchorNode)) {
        ...
        if (points.length && points.every((point) => point === points[0]) && (points[0] === 8 || points[0] === 10 || points[0] === 12)) {
          fontSize = `${points[0]}pt` as '8pt' | '10pt' | '12pt';
        }
      }
  ...
  const applyFontSize = (fontSize: '8pt' | '10pt' | '12pt') => {
    if (!editorRef.current) return;
    restoreSavedSelection();
    try {
          ...
          <div className="rich-text-toolbar sticky top-0 z-30 bg-slate-100/95 backdrop-blur-xs border-b border-slate-200/90 px-1.5 py-0.5 flex items-center gap-0.5 overflow-x-auto no-scrollbar touch-pan-x text-slate-700 select-none shrink-0">
            {/* Riwayat Undo/Redo */}
              ...
              <select
                ...
                onChange={(e) => applyFontSize(e.target.value as '8pt' | '10pt' | '12pt')}
