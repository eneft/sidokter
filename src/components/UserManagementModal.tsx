...
export const UserManagementModal: React.FC<UserManagementModalProps> = ({
  ...
  const handleDeleteClick = (user: UserAccount) => {
    if (user.username === 'admin' || user.id === 'admin-root') {
      onShowToast('error', 'Akses Ditolak', 'Akun Administrator Root utama tidak dapat dihapus demi keamanan sistem.');
      return;
    }
      ...
      <div className="bg-white w-full max-w-6xl rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-slate-50 p-3.5 rounded-xl border border-slate-200">
            ...
            <AdminTooltip content="Daftarkan akun login baru untuk staf medis, staf penunjang, pejabat struktural, atau verifikator dokumen." title="Tambah Petugas Baru">
              <button
              ...
              <form onSubmit={handleFormSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      disabled={editingUserId === 'admin-root' || (!!editingUserId && username.trim().toLowerCase() === 'admin')}
                      placeholder="contoh: user_pelayanan"
                      className="w-full bg-white border border-slate-300 rounded-lg pl-8 pr-3 py-2 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:bg-slate-100 disabled:text-slate-500 disabled:cursor-not-allowed"
                      required
                    />
                  </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label className="text-xs font-semibold text-slate-700 flex items-center gap-1">
                      Role / Hak Akses
                      <AdminHelpHint text="User biasa hanya dapat mengakses dokumen unit yang ditugaskan. Admin memiliki kendali penuh atas portal administrasi, master data, dan pemeliharaan sistem." />
                    </label>
                    {(editingUserId === 'admin-root' || username.trim().toLowerCase() === 'admin') && (
                  ...
                  <select
                    value={role}
                    disabled={editingUserId === 'admin-root' || username.trim().toLowerCase() === 'admin'}
                    onChange={(e) => {
                  ...
                  {(editingUserId === 'admin-root' || username.trim().toLowerCase() === 'admin') && (
                    <p className="text-[10px] text-slate-400 mt-1">Akun Administrator Root SIDOKTER terlindungi agar sistem selalu memiliki akses manajemen utama.</p>
                  )}
                </div>
                ...
                <div className="sm:col-span-2 space-y-2">
                  <label className="text-xs font-semibold text-slate-700 flex items-center gap-1">
                    ...
                    <AdminHelpHint text="STRUKTURAL memberikan hak akses SK & MOU serta tandatangan pimpinan. VERIFIKATOR berwenang memvalidasi dan mempublikasikan SPO unit." />
                  </label>
                        ...
                        <tr key={u.id} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="sticky right-0 z-10 px-2 py-2 bg-white border-l border-slate-200">
                            <div className="flex items-center justify-center gap-1">
                              <AdminTooltip content="Ubah profil akun, mutasi penugasan hierarki, ganti badge, atau ubah kata sandi." title="Edit Data Akun" side="left">
                                <button type="button" onClick={() => handleOpenEditForm(u)} className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-[11px] font-bold text-indigo-700 bg-indigo-50 border border-indigo-200 hover:bg-indigo-100 rounded-lg transition-colors cursor-pointer">
                                  <Edit3 className="w-3.5 h-3.5" /><span>Edit</span>
                                </button>
                              </AdminTooltip>
                              {u.username !== 'admin' && u.id !== 'admin-root' && (
                                <AdminTooltip content="Hapus akun pengguna ini secara permanen dari sistem database." title="Hapus Akun" side="left">
                                  <button type="button" onClick={() => handleDeleteClick(u)} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors cursor-pointer">
                ...
                return (
                  <article key={u.id} className="border border-slate-200 rounded-xl p-3.5 bg-white shadow-sm">
                    ...
                    <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-end gap-2">
                      ...
                      {u.username !== 'admin' && u.id !== 'admin-root' && <button type="button" onClick={() => handleDeleteClick(u)} className="inline-flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-semibold text-rose-600 bg-rose-50 border border-rose-200 hover:bg-rose-100 rounded-lg cursor-pointer"><Trash2 className="w-3.5 h-3.5" /> Hapus</button>}
                    </div>
                  </article>
                );
              })}
